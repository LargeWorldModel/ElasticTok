...
def main(argv):
    ...
    with mesh:
        ...
        for i in range(n_chunks):
            ...
            recon, recon_loss, final_ntoks, z = inference.inference(
                params, batch, FLAGS.threshold, FLAGS.default_prop_codes, FLAGS.max_prop_codes
            )
            recon, recon_loss, final_ntoks  = jax.device_get((recon, recon_loss, final_ntoks))
            print(f'Chunk {i} / {n_chunks}')
            print('\tRecon Losses:', recon_loss[0])
            print(f'\tTokens Used (Max {elastic_config.max_toks}):', final_ntoks[0])
            zs.extend(z[0])
